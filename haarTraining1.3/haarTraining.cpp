	...
	const char* featdirname,const char* resultname, int num_pos, int num_neg,int numsplits,int equalweights,const char* dirname, float minhitrate, float maxfalsealarms)
{
	...
	while (T <= 5)
	{
		...
		while(getline(istream, str))
		{
			...
			for (int k = 0;k < threArray.size() - 1;k++)
			{
				...
				for (int n = 0;n < sampleNumber;n++)
				{
					...
					if (object == 1)
					{
						tp = tp + haarTrainingData->weights.data.fl[idx[n]] * 1.0;   //计算T+
						if (vector_feat[n] < threshold)
							sp = sp + haarTrainingData->weights.data.fl[idx[n]] * 1.0;   //计算S+
					}
					else
					{
						tn = tn + haarTrainingData->weights.data.fl[idx[n]] * 1.0;    //计算T-
						if (vector_feat[n] < threshold)
							sn = sn + haarTrainingData->weights.data.fl[idx[n]] * 1.0;  //计算S-
					}				
		...
		*/
		...
		cout << T << "结束,耗时：" << (end - start) / CLOCKS_PER_SEC * 1000 << endl;
		T++;
		istream.close();
	}
	//保存成xml文件
	...
	int maxtreesplits, int minpos, bool bg_vecfile,bool pos_vecfile)
{
	...
	cout << "样本收集耗时：" << (read_image_time - start) / CLOCKS_PER_SEC * 1000 << "ms" << endl;
...
//	cout << "直接计算" << endl;
	...
	cout << "特征计算（排序）耗时：" << (calc_feature_time - read_image_time) / CLOCKS_PER_SEC * 1000 << "ms" << endl;
